...
use poc_framework_v3::Hmac;
use hex;
use crate::{
    debugger::get_host_ptr,
    ebpf,
    ebpf::STACK_PTR_REG,
...
impl<'a, 'b, V: Verifier, E: UserDefinedError, I: InstructionMeter> Interpreter<'a, 'b, V, E, I> {
    ...
    pub fn get_vm_address(&self, address: usize) -> u64 {
        ((address * ebpf::INSN_SIZE) as u64)
            + self
                .vm
                .verified_executable
                .get_executable()
            ...
            ebpf::CALL_IMM   => {
                ...
                if syscalls {
                    if let Some(syscall) = executable.get_syscall_registry().lookup_syscall(insn.imm as u32) {
                        resolved = true;
                        //println!("aaaA: {:?}", syscall);
                        //println!("pc: {:X}", self.get_vm_address(self.pc));
                ...
                if calls {
                    if let Some(target_pc) = executable.lookup_bpf_function(insn.imm as u32) {
                        resolved = true;
                        //println!("call from: {:X} to {:X} || PC: {:X}", self.get_vm_address(self.pc), self.get_vm_address(target_pc), self.get_vm_address(pc));

                        if self.get_vm_address(self.pc) == 0x3BBE8 && self.get_vm_address(target_pc) == 0x7C5C8 {
                            let start_addr = 0x200008ed8;
                            let mut data = [0u8; 32];
                            ...
                            self.vm.hmac.update_result(data);
                            return Ok(Some(0));
                            //println!("HMAC {:?}", hex::encode(data));
                        }
