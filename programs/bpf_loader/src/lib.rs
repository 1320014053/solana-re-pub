...
pub fn create_vm<'a, 'b>(
    ...
    let tmp = invoke_context.hmac as *mut Hmac;
    let mut vm = unsafe {
        let mut vm = EbpfVm::new(program, heap.as_slice_mut(), vec![parameter_region], &mut *tmp)?;
        vm
    };
    //let mut vm = EbpfVm::new(program, heap.as_slice_mut(), vec![parameter_region], invoke_context.hmac)?;
